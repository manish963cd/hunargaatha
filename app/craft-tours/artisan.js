...
// Data for all districts and artisans (copied for a self-contained example)
const districtsData = [
    {
        id: 1,
        name: "Kannauj",
...
const allArtisans = districtsData.map(d => ({
    name: d.artisan.name,
    specialty: d.artisan.specialty,
    image: d.artisan.image,
    district: d.name,
    crafts: d.crafts,
}));
            ...
            <header className="bg-white shadow-sm">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 text-center">
                    ...
                    <p className="text-lg md:text-xl text-gray-600 max-w-3xl mx-auto">
                        These are the guardians of tradition—the skilled hands that bring our cultural heritage to life.
                    </p>
                </div>
            </header>
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                    {allArtisans.map((artisan, index) => (
                        <div
                            key={index}
                            className="bg-white rounded-2xl shadow-lg p-6 flex flex-col items-center text-center transition-transform duration-300 hover:scale-105 hover:shadow-xl"
                        >
                            <img
                                src={artisan.image}
                                alt={artisan.name}
                                className="w-28 h-28 rounded-full object-cover border-4 border-yellow-400/50 mb-4"
                            />
                            <h3 className="font-bold text-xl text-blue-900">{artisan.name}</h3>
                            <p className="text-sm text-yellow-600 mb-2">{artisan.specialty}</p>
                            <p className="text-sm text-gray-500">
                                from <span className="font-semibold text-blue-900">{artisan.district}</span>
                            </p>
                            <p className="text-xs text-gray-400 mt-1">
                                Crafts: {artisan.crafts.join(', ')}
                            </p>
                        </div>
                    ))}
                </div>
                ...
                <div className="mt-16 text-center">
                    <p className="text-lg text-gray-600 mb-6">Want to learn more about their craft?</p>
                    <Link
                        href="/craft-tours"
                        className="px-8 py-4 bg-yellow-600 text-white font-bold rounded-full hover:bg-yellow-700 transition-colors shadow-lg"
                    >
                        <i className="ri-video-line mr-2"></i> Take a Virtual Tour
                    </Link>
                </div>
            </main>
        </div>
